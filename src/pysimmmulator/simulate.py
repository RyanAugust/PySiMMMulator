...
class Simulate(Visualize):
    ...
    def simulate_baseline(
        ...
        seasonality = (self.rng.normal(loc=self.baseline_params.temp_coef_mean,
                                       scale=self.baseline_params.temp_coef_sd,
                                       size=1) * temp)
        # add some noise to the trend
        error = self.rng.normal(loc=0,
    ...
    def simulate_ad_spend(
        ...
        campaign_count = int(self.basic_params.years * 365 /
        ...
        for channel in max_min_proportion_on_each_channel.keys():
            ...
            spend_df[channel] *= (
                ((self.baseline_sales_df["trend"] / self.baseline_sales_df["base"]) + 1
    ...
    def simulate_media(self, true_cpm: dict, true_cpc: dict, noisy_cpm_cpc: dict) -> None:
        ...
        for channel in media_params.noise_channels:
            ...
            channel_noisy_cpm_value = (true_cpm[channel] +
                                       channel_noise if channel in true_cpm.keys() else np.nan)
            ...
            channel_noisy_cpc_value = (true_cpc[channel] +
                                       channel_noise if channel in true_cpc.keys() else np.nan)
            ...
            self._negative_check(self.spend_df.loc[channel_idx],
                                 column="noisy_cpm",
                                 channel=channel)
            self._negative_check(self.spend_df.loc[channel_idx],
                                 column="noisy_cpc",
        ...
        self.spend_df["daily_impressions"] = np.round(
            self.spend_df["lifetime_impressions"] / self.basic_params.frequency_of_campaigns,
            0,
        )
    ...
    def simulate_cvr(self, noisy_cvr: dict) -> None:
        ...
        for channel in cvr_params.noise_channels:
            ...
            channel_noise = self.rng.weibull((1 / noisy_cvr[channel]["scale"]) / 10 + 1,
                                             size=len(channel_idx))
            self.spend_df.loc[channel_idx,
                              "noisy_cvr"] = (channel_noise * self.basic_params.true_cvr[channel])
    ...
    def _reformat_for_mmm(self) -> None:
        ...
        Returns:
            None
        """
        date_backbone = pd.date_range(start=self.basic_params.start_date,
                                      end=self.basic_params.end_date,
                                      freq="D")
        campaigns_in_period = (date_backbone.shape[0] / self.basic_params.frequency_of_campaigns)
        campaign_id_to_date_map = np.trunc(
            np.linspace(start=0, stop=campaigns_in_period - 1,
                        num=date_backbone.shape[0])).astype(int)
    ...
    def _simulate_diminishing_returns(
        ...
        for channel in alpha_saturation.keys():
            ...
            x_scurve = target**alpha_saturation[channel] / (target**alpha_saturation[channel] +
    ...
    def simulate_decay_returns(self, true_lambda_decay: dict, alpha_saturation: dict,
                               gamma_saturation: dict) -> None:
    ...
    def consolidate_dataframe(self) -> None:
        ...
        self.mmm_df["total_revenue_from_ads"] = (self.mmm_df["total_conversions_from_ads"] *
                                                 self.basic_params.revenue_per_conv)
        self.mmm_df["baseline_revenue"] = (round(self.baseline_sales_df["baseline_sales"]) *
                                           self.basic_params.revenue_per_conv)
        self.mmm_df["total_revenue"] = self.mmm_df[["total_revenue_from_ads",
                                                    "baseline_revenue"]].sum(axis=1)
    ...
    def calculate_channel_roi(self) -> None:
        ...
        self.channel_roi = {}
        for channel in self.basic_params.all_channels:
            total_cpa = (self.mmm_df[f"{channel}_spend"].sum() /
    ...
    def run_with_config(self, config: dict) -> set[pd.DataFrame, dict]:
        if self.basic_params is None:
            self.basic_params = BasicParameters(**config["basic_params"])
