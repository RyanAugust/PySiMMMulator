...
class Simulate(Visualize):
  ...
  def simulate_baseline( self, base_p: int, trend_p: int, temp_var: int, temp_coef_mean: int, temp_coef_sd: int, error_std: int,) -> pd.DataFrame:
    ...
    Returns:
      pd.DataFrame: DataFrame containing baseline sales"""
    ...
    # if error term makes baseline_sales negative, use truncated normal to resample rather than simple clamping
    if np.any(baseline_sales < 0):
        # This is a bit complex as we have a vector sum. 
        # For simplicity and to avoid mean shift, we resample the error term 
        # such that baseline_sales stays non-negative where possible, 
        # but a simple where/clamp is still used as a final safety if base+trend+seasonality is already < 0.
