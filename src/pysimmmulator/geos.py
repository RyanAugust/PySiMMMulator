...
class Geos:
    ...
    def __call__(self,
                 ...
                 universal_scale: Optional[float] = 1.0,
                 count: int = 250) -> dict:
        ...
        if geo_specs is not None:
            return self.create_geos(geo_specs=geo_specs, universal_scale=universal_scale)
    ...
    def _invent_geos(self, name_length: int = 12, count: int = 250) -> dict:
        ...
        while len(geo_specs) < count:
            geo_specs.update({self._namer(length=name_length): {"pop_pct": pop_pcts_list.pop()}})
        return geo_specs
    ...
    def create_random_geos(self, count: int = 250) -> dict:
        ...
        for geo in geo_details.keys():
            geo_details[geo] = int(geo_details[geo]["pop_pct"] * self.total_population)
        return geo_details

def distribute_to_geos(
    ...
    for geo_name, geo_pop in geo_details.items():
        ...
        if any(media_cost_spec) != 0.0:
            geo_dataframe[media_cols] *= (
                1 + abs(rng.normal(loc=pop_pct * media_cost_spec[0], scale=media_cost_spec[1])))
        if any(perf_spec) != 0.0:
            geo_dataframe["total_revenue"] *= (
                1 + abs(rng.normal(loc=pop_pct * perf_spec[0], scale=perf_spec[1])))
